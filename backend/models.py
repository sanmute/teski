...
# Guard against double-loading under both ``models`` and ``backend.models``.
# If we are imported as anything other than ``models``, delegate to the
# canonical module so SQLModel tables register exactly once.
if __name__ != "models":
